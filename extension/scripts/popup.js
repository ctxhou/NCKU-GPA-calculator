...
function initGPABookmarklet() {
    var gpaTotal = 0,
        creditTotal = 0;
    var coreGenTotal = [0, 0, 0, 0];
    var overGenTotal = [0, 0, 0, 0];
...
function showResult(gpaScore, allClass, semesterNames, coreGen, overGen) {
    ...
    for (var key in allClass) {
        ...
        for (var detail in allClass[key]) {
            tbody = tbody + "<tr><td>" + allClass[key][detail].className + "</td><td>" + allClass[key][detail].credit + "</td><td>" + allClass[key][detail].score + "</td><td>" + allClass[key][detail].gen + "</td></tr>"
        }
    }
    $('body').append("<div id='my-score' style='padding: 10px;background-color:#f7f7f7; position:absolute; left: 25%; top: 0px'><button id='close'>close</button>" + header + "<table>" + thead0 + tbody0 + "</table>" + "<table style='margin-top: 20px;'>" + thead + tbody + "</table> </div>")
